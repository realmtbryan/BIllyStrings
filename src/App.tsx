...
export default function App() {
  ...
  // Settings State (Nugs Integration)
  ...
  const filteredShows = useMemo(() => {
    return SHOWS.filter(show => {
      ...
      // Priority: Drive -> Recap -> YouTube -> Date
      const typeOrder = { 'drive': 1, 'recap': 2, 'youtube': 3 };
      const orderA = typeOrder[a.sourceType || 'youtube'];
      const orderB = typeOrder[b.sourceType || 'youtube'];
    ...
    <div className="min-h-screen bg-[#0a0a0a] text-gray-200 font-sans selection:bg-red-500/30">
      ...
      <div className="flex">
        ...
        <aside className="w-64 hidden lg:flex flex-col sticky top-14 h-[calc(100vh-3.5rem)] border-r border-white/5 bg-[#0d0d0d]">
          <div className="p-4 border-b border-white/5">
             <button 
                onClick={() => setActiveFilter({year: null, month: null})}
                className={`w-full flex items-center gap-3 px-3 py-2 rounded-lg text-sm font-semibold transition-colors ${
                  activeFilter.year === null ? 'bg-red-600/10 text-red-500' : 'text-gray-400 hover:bg-white/5 hover:text-white'
                }`}
             >
                <Home className="w-4 h-4" /> All Shows
             </button>
          </div>
          ...
          <div className="p-6">
            <div className="mb-6 flex items-end justify-between">
              <div>
                <h1 className="text-2xl font-bold text-white mb-1">
                  {activeFilter.year === null ? 'All Archives' : activeFilter.month === null ? `${activeFilter.year} Tours` : `${getMonthName(activeFilter.month!)} ${activeFilter.year}`}
                </h1>
            ...
            {filteredShows.length === 0 && (
              <div className="flex flex-col items-center justify-center py-32 border border-dashed border-white/10 rounded-2xl bg-white/5">
                ...
                <p className="text-sm text-gray-500 mt-1">Try selecting a different month or adjusting your search.</p>
              </div>
            )}
          </div>
        </main>
      ...
      <AnimatePresence>
        {selectedShow && (
          ...
          >
            <div className="max-w-6xl mx-auto min-h-screen p-4 md:p-8 relative">
              <button 
              ...
              <div className="space-y-6 max-w-4xl mx-auto mt-12 md:mt-4">
                ...
                {nugsToken && (
                  <div className="bg-gradient-to-r from-green-900/20 to-[#121212] border border-green-500/30 rounded-2xl p-5">
                    ...
                    {/* Placeholder Audio Player for when we map the specific Nugs Catalog IDs */}
                ...
                {selectedShow.setlist && (
                  <div className="bg-[#121212] rounded-2xl p-6 border border-white/5 mt-8 shadow-lg">
